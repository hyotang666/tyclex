...
(defun subtype?(t1 t2)
  (labels((entry-point(t1 t2)
	  ...
	  (both-ts(t1 t2)
	    (if(subtypep t1 t2)
	      (multiple-value-call #'t1-is-subtype-of-t2
		t1 (millet:type-expand t1)t2(millet:type-expand t2))
	      (t1-is-not-subtype-of-t2 t1 t2)))
	  (t1-is-subtype-of-t2(t1 _ expanded?1 t2 expanded expanded?2)
	    (declare(ignorable _))
	    (if expanded?1
	      (tsexp/- t1 t2 expanded?2)
	      (ts!exp/- t2 expanded?2 expanded)))
	  (tsexp/-(t1 t2 expanded?2)
	    (if expanded?2
