  ...
  (if(millet:type-specifier-p t1)
	...
	(let((expanded?1 (nth-value 1(millet:type-expand t1))))
	  (multiple-value-bind(expanded expanded?2)(millet:type-expand t2)
		...
		T))))
	(or (eql t t1)
	    #+sbcl(when(eq 'sb-kernel:simple-character-string t2)
		    (subtype? t1 'string))
	    ))
      (if(adt-p t2)
	...
	(if (newtypep t1)
	  (if (newtypep t2)
	    (eq (alexandria:ensure-car t1)(alexandria:ensure-car t2))
	    (subtype? (millet:type-expand t1) t2))
	  (if (newtypep t2)
	    (subtype? t1 (millet:type-expand t2))
	    (or (eql t t1)
	...
	(if (newtypep t1)
	  (if (newtypep t2)
	    (eq (alexandria:ensure-car t1)(alexandria:ensure-car t2))
	    (subtypep (millet:type-expand t1) t2))
	  (if (newtypep t2)
	    (subtypep t1 (millet:type-expand t2))
	    (or (eql t t2)
