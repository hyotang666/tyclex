...
(defmacro tcl::defun (name lambda-list &body body &environment env)
  (if (or (listp name) ; setf form, ignore.
      ...
      (multiple-value-bind (body decls doc)
        ...
        (let ((return-type
               (tyclex.compute-return-type:compute-return-type
                 (car (last body))
                 (if decls
                     ...
                     env))))
          `(progn
            (tcl::declaim (ftype (function * ,return-type) ,name))
            (cl:defun ,name ,lambda-list ,@doc ,@decls ,@body))))
      (let ((types
             (second
               (or (tyclex.curry:function-type-of name)
                   (introspect-environment:function-type name)))))
        (multiple-value-bind (body decls doc)
            (alexandria:parse-body body :documentation t)
  ...
  (labels ((entry-point (lambda-list types &optional acc)
           ...
           (body (elt rest types acc)
             (if (lambda-fiddle:lambda-keyword-p elt)
                 ...
                 (entry-point rest (cdr types)
                              (acc-declare elt (car types) acc))))
           ...
           (acc-declare (var type acc)
             (if (find type '(t *))
                 acc
                 (cons `(declare (type ,type ,var)) acc)))
           ...
           (optional-body (rest types acc)
             (if (endp rest)
                 ...
                 (if (lambda-fiddle:lambda-keyword-p (car rest))
                     ...
                     (optional-body (cdr rest) (cdr types)
                                    (acc-declare
                                      (alexandria:ensure-car (car rest))
                                      (car types)
                                      (if (typep (car rest)
                                                 '(cons symbol
                                                        (cons *
                 ...
                 (if (lambda-fiddle:lambda-keyword-p (car rest))
                     ...
                     (key (cdr rest) types
                          (multiple-value-bind (var key)
                              (ensure-var (car rest))
                            (acc-declare var (some (compare key) types)
                                         acc))))))
           (ensure-var (thing)
             (etypecase thing
           ...
           (&rest (rest types acc)
             (if (endp (cdr rest))
                 (acc-declare (car rest) (find-&rest-type types) acc)
                 (diverge (cadr rest) (cddr rest) types
                          (acc-declare (car rest) (find-&rest-type types)
                                       acc))))
                            ...
                            (mapcan
                              (lambda (bind)
                                (when (and (listp bind)
                                           (cadr bind)
                                           (not
                                             (and (introspect-environment:specialp
                                                    (car bind))
                                                  (eq (find-package :cl)
                                                      (symbol-package
                                                        (car bind))))))
                                  `((,(car bind)
                                     ,(tyclex:compute-return-type (cadr bind)
                                                                  env)))))
                              bind*))
       ,@body)))
    ...
    ,@(reduce
        (lambda (acc elt)
          (if (some
                ...
                decls)
              acc
              (cons `(type ,(second elt) ,(first elt)) acc)))
        bind-types
        :initial-value decls)))
     ...
     ,@(loop :for decl :in decls
             :when (ftype-function-formp decl)
               :collect (destructuring-bind
                            (ftype (function arg return) name)
                            decl
...
(defmacro tcl::defpackage (name &rest args)
  `(eval-when (:compile-toplevel :load-toplevel :execute)
     (handler-bind (#+sbcl
                    (sb-ext:name-conflict (restart-invoker 'sb-impl::take-new))
  ...
  (let (import export)
    (do-external-symbols (symbol package)
      (multiple-value-bind (s status)
          (find-symbol (symbol-name symbol) :tcl)
        (if status
            (pushnew s export :test #'string=)
            (progn (push symbol import) (push symbol export)))))
    (import import :tcl)
    (export export :tcl)))
