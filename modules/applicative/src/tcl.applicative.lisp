...
(defmacro lift (function &rest functor*) `(<$> ,function ,@functor*))

(defun lift* (function &rest functor*) (eval `(<$> ,function ,@functor*)))

(defmacro sequence-a (applicative*)
  (trivia:ematch applicative*
    ((null) `(pure nil))
    ((cons x xs) `(<$> (curry cons _ _) ,x (sequence-a ,xs)))))

(defun sequence-a* (applicative*) (eval `(sequence-a ,applicative*)))
...
(definstance (applicative list)
  ((<*> (list arg)
     (alexandria:with-gensyms (f)
       `(mapcan (lambda (,f) (mapcar ,f ,arg)) ,list)))
   (pure (x) `(list ,x))))

(definstance (applicative function)
  ((pure (x) `(lambda (#0=#:arg) (declare (ignore #0#)) ,x))
   (<*> (f g)
     (alexandria:with-gensyms (x)
       `(lambda (,x) (funcall (funcall ,f ,x) (funcall ,g ,x)))))))

(definstance (applicative io)
  ((<*> (functor arg)
     `(make-instance 'io-action
                     :instance (lambda ()
                                 (funcall (funcall ,functor) (funcall ,arg)))
                     :type '(io *)))
   (pure (x)
