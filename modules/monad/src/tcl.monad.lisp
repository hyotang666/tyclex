...
(define-type-class (monad m) ()
  ((return (a) (m a))
   ...
   (>> ((m a) (m b)) (m b))
   (fail (string) (m a)))
  (:default >> (x y) `(>>= ,x (lambda (#0=#:arg) (declare (ignore #0#)) ,y)))
  (:default fail (msg) `(error ,msg)))
...
(definstance (monad list)
  ((return (x) `(list ,x))
   (>>= (xs f) `(mapcan ,f ,xs))
   (fail (x) (declare (ignore x)) nil)))

(definstance (monad function)
  ((return (x) `(lambda (#0=#:_) (declare (ignore #0#)) ,x))
   (>>= (h f) `(lambda (#1=#:w) (funcall (funcall ,f (funcall ,h #1#)) #1#)))))

(definstance (monad io)
  ((return (x)
     `(make-instance 'io-action :instance (lambda () ,x) :type '(io *)))
   (>>= (io fun) `(funcall ,fun (funcall ,io)))))
  ...
  ((return (x) `(just ,x))
   (>>= (m f)
     (trivia:match m
       ...
       ((just x) `(funcall ,f ,x))
       (_
        (alexandria:with-gensyms (x g)
          `(trivia:ematch* (,m ,f)
             ((nothing _) nothing)
             (((just ,x) ,g) (funcall ,g ,x)))))))
   (fail (a) (declare (ignore a)) 'nothing)))