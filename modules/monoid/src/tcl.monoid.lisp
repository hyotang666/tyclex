...
(define-type-class (monoid m) ()
  ((mempty () m)
   ...
   (mconcat ((list m)) m))
  (:default mconcat (ms)
    `(reduce (lambda (a m) (mappend a m)) ,ms
             :from-end t
...
(definstance (monoid vector)
  ((mempty () `(vector))
   (mappend (a b) `(concatenate 'vector ,a ,b))))

(definstance (monoid string)
  ((mempty () "")
   (mappend (a b) `(concatenate 'string ,a ,b))))

(definstance (monoid bit-vector)
  ((mempty () `(make-array 0 :element-type 'bit))
   (mappend (a b) `(concatenate 'bit-vector ,a ,b))))

(definstance (monoid (product number))
  ((mempty () `(product 1))
   (mappend (a b) `(product (* ,a ,b)))))
  ...
  ((mempty () nothing)
   (mappend (a b)
     (trivia:match* (a b)
       ...
       ((_ _)
        (alexandria:with-gensyms (m m1 m2)
          `(trivia:ematch* (,a ,b)
             ((nothing ,m) ,m)
             ((,m nothing) ,m)
             (((just ,m1) (just ,m2)) (just (eval `(mappend ,,m1 ,,m2)))))))))))
