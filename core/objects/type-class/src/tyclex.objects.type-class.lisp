...
(defstruct (type-class (:copier nil) (:conc-name type-))
  (name (error 'tyclex.conditions:slot-uninitialized :name 'name) :type
   (or symbol list) :read-only t)
  (vars (error 'tyclex.conditions:slot-uninitialized :name 'vars) :type list
   :read-only t)
