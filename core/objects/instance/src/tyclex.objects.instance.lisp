...
(defstruct
    (instance (:predicate nil) (:constructor make-type-class-instance)
     (:copier nil))
  ...
  (definitions (error 'tyclex.conditions:slot-uninitialized :name 'definitions)
   :type list :read-only t)
  (types (error 'tyclex.conditions:slot-uninitialized :name 'types) :type list
   :read-only t)
  (constraints (error 'tyclex.conditions:slot-uninitialized :name 'constraints)
   :type list :read-only t))

(defun instance= (x y) (equal (instance-signature x) (instance-signature y)))
